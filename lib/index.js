...
/**
 * Default argument template, deliberately shaped like terminal-notifier's
 * `-title` / `-message`. Only consulted when `system.notifier.command` is set,
 * so it never changes behaviour for users who leave the escape hatch alone.
 */
export const NOTIFIER_DEFAULT_ARGS = Object.freeze(["-title", "{{title}}", "-message", "{{body}}"]);
...
export const DEFAULT_CONFIG = Object.freeze({
  ...
  system: Object.freeze({
    enabled: false,
    sound: true,
    notifier: Object.freeze({ command: "", args: NOTIFIER_DEFAULT_ARGS }),
  }),
  ...
  return z.object({
    ...
    system: z.object({
      ...
      notifier: z.object({
        ...
        args: z.array(z.string()).default([...NOTIFIER_DEFAULT_ARGS]),
      }),
    }),
    security: z.object({
 ...
 * unusable `command` just means "use the OS default", and an empty or non-string
 * `args` list falls back to the documented template rather than producing a
 * notifier that is configured but silently runs nothing.
 */
function cleanNotifier(value) {
  ...
  return {
    ...
    // An explicit `[]` means "this notifier takes no arguments" and must be
    // honoured; only a malformed `args` falls back to the documented template.
    args: Array.isArray(source.args) ? args : [...NOTIFIER_DEFAULT_ARGS],
  };
}
